...
var logoscale = lerp(2,1,openlerp+breath*0.01);
var logooffset = lerp(64,24,openlerp);
...
for (var i = 0; i < modelen; i++) {
	...
	var ydraw = yoffset + ybegin + i * (lineheight + ygap);
	ydraw+=8;
...
prompt += "Press F4 (keyboard) or down + any three buttons (controller) to rebind controls | ";
if (breath > 0) prompt += "Press Start/[A] to confirm | Press Select/[B] to cancel";
else            prompt += "Press [A=SLASH] to confirm | Press [B=SHOOT] to cancel";
